...
PlayAreaReservation & play\_area\_reservations & FK(play\_area\_id), CHECK(end > start), game\_type VARCHAR, version INTEGER \\
...
\subsection{Operational Deployment Status}
The monorepo structure is fully operational using npm workspaces. The CI/CD pipeline is active at \texttt{.github/workflows/ci.yml}, incorporating automated build, lint, and test scripts along with multi-stage Docker build steps using the optimized \texttt{type=gha} Docker buildx caching provider. Centralized observability bootstrapping is active at \texttt{backend/src/shared/telemetry/otel-bootstrap.ts}.
...
\subsection{Operational Deployment Status}
The identity, authentication, and team management logic is fully implemented under \texttt{src/modules/identity/}. The secure 4-digit PIN authentication verification is integrated directly into the rich \texttt{User} domain model. Access control is enforced via custom \texttt{@Roles} metadata annotations and framework guards (\texttt{JwtAuthGuard}, \texttt{RolesGuard}). The relational database schema in \texttt{schema.sql} has been updated to include indices, \texttt{teams}, and \texttt{team\_rosters} layout mappings.
...
\section{Phase 8: Frontend (Capacitor PWA, React, TypeScript, Zustand)}

\subsection{Technical Stack Constraints}
\begin{itemize}
    \item \textbf{Client Engine}: React 18+, TypeScript, Vite, Zustand, and TanStack Query.
    \item \textbf{Deployment Packaging}: Hybrid packaging via CapacitorJS wrappers to ensure zero-latency native bridge communication and WebKit web views.
\end{itemize}

\subsection{Phase 8a: UI/UX Guidelines, Typography, and Theme Design Tokens}
Implement a strict, high-contrast palette with mascot logo design tokens:
\begin{itemize}
    \item \textbf{Dark Base / Surfaces}: Rich Black (\texttt{\#000000} / \texttt{\#111111})
    \item \textbf{Primary Brand Container / Secondary Surfaces}: Midnight Sports Teal (\texttt{\#0A4454})
    \item \textbf{High-Contrast Accent / Active States}: Pale Electric Yellow (\texttt{\#F9E06B})
    \item \textbf{Dynamic Warning / Core Highlights}: Emblem Gold (\texttt{\#F5C53B})
    \item \textbf{Text / Crisp Foreground Contrast}: Pure White (\texttt{\#FFFFFF})
\end{itemize}

\subsection{Phase 8b: App Navigation Shell \& Layout Boundaries}
Define application navigation layout:
\begin{itemize}
    \item App navigation shell mapping core tab views.
    \item Strict layout boundaries for desktop web and mobile Capacitor screen viewport frames.
    \item Differentiated layouts for Public and Private profile structures.
\end{itemize}

\subsection{Phase 8c: Authentication Guardrails \& State Synchronization}
Implement frontend credentials lifecycle:
\begin{itemize}
    \item WebKit cookie fallback handles HTTP secure cookies on Capacitor mobile containers.
    \item Session state synchronized using Zustand stores with persisted client sync.
    \item Client-side route guards enforcing authentication transitions.
\end{itemize}

\subsection{Phase 8d: Real-Time Feature Synchronization \& Matchmaking}
Integrate real-time socket events:
\begin{itemize}
    \item Matchmaking queue lobbies with status feedback loops.
    \item Score-reporting dynamic sheets validating input values against tournament and match rules.
    \item Service Worker caching routines and Capacitor integration for mobile packaging.
\end{itemize}
